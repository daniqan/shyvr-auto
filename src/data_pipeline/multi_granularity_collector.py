...
from src.data_pipeline.model_specific_exporter import ModelSpecificExporter
...
class MultiGranularityCollector(InitialCorpusCollector):
    ...
    async def export_model_specific_formats(
        self,
        corpus_data: Dict[str, pd.DataFrame],
        ...
        Args:
            corpus_data: Dictionary of DataFrames with OHLCV and features
            output_dir: Directory to save files (default: data/corpus/v2.0/model_ready)
            model_types: List of model types to export for (default: all)
            
        Returns:
            Nested dictionary: {model_type: {dataset_key: filepath}}
        """
        if output_dir is None:
            output_dir = Path("data/corpus/v2.0/model_ready")
        
        exporter = ModelSpecificExporter()
        
        if model_types:
            # Export for specific models
            all_exports = {}
            for model_type in model_types:
                logger.info(f"Exporting sequences for {model_type} model")
                exports = exporter.export_for_model(corpus_data, model_type, output_dir)
                all_exports[model_type] = exports
        else:
            # Export for all models
            logger.info("Exporting sequences for all model types")
            all_exports = exporter.export_all_models(corpus_data, output_dir)
        
        # Log summary
        for model_type, exports in all_exports.items():
            total_sequences = sum(1 for _ in exports.values())
            logger.info(f"Exported {model_type} format",
                       datasets=len(exports),
                       total_sequences=total_sequences)
        
        return all_exports
    
    async def export_model_formats_to_gcs(
        self,
        corpus_data: Dict[str, pd.DataFrame],
        bucket_name: str = "shyvr-models-prod",
        gcs_prefix: str = "training-data/model-ready/v2.0",
        model_types: Optional[List[str]] = None
    ) -> Dict[str, Dict[str, str]]:
        """
        Export model-specific formats to Google Cloud Storage
        
        Args:
            corpus_data: Dictionary of DataFrames with OHLCV and features
            bucket_name: GCS bucket name
            gcs_prefix: Prefix for GCS paths
            model_types: List of model types to export (default: all)
            
        Returns:
            Nested dictionary: {model_type: {dataset_key: gcs_path}}
        """
        # First export to local files
        local_exports = await self.export_model_specific_formats(
            corpus_data, model_types=model_types
        )
        ...
        for model_type, model_files in local_exports.items():
            gcs_exports[model_type] = {}
            
            for dataset_key, local_path in model_files.items():
                # Create GCS path
                gcs_path = f"{gcs_prefix}/{model_type}/{local_path.name}"
                blob = bucket.blob(gcs_path)
                
                # Upload file
                blob.upload_from_filename(str(local_path))
                
                gcs_exports[model_type][dataset_key] = f"gs://{bucket_name}/{gcs_path}"
                
                logger.info(f"Uploaded {model_type} sequences for {dataset_key}",
                           gcs_path=gcs_exports[model_type][dataset_key],
                           size_mb=local_path.stat().st_size / 1024 / 1024)
        
        # Upload export metadata
        metadata_path = Path("data/corpus/v2.0/model_ready/export_metadata.json")
        if metadata_path.exists():
            metadata_blob = bucket.blob(f"{gcs_prefix}/export_metadata.json")
            metadata_blob.upload_from_filename(str(metadata_path))
            logger.info(f"Uploaded export metadata to gs://{bucket_name}/{gcs_prefix}/export_metadata.json")
